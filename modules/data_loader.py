...
class GetDataframe:
    ...
    - validate_data_dir (staticmethod): статический метод для проверки, что data_dir 
                                   является строкой и существующим путем к директории.
    ...
    def __init__(self, data_dir):
        ...
        """
        self.data_dir = self.validate_data_dir(data_dir)

    @staticmethod
    def validate_data_dir(data_dir):
        """
        Статический метод для проверки, что data_dir является строкой и существующим путем к директории.

        Параметры:
        - data_dir (str): директория с данными для датасета.

        Возвращает:
        - data_dir (str): проверенный путь к директории.
        """
        if not isinstance(data_dir, str):
            raise ValueError(f"Значение переменной data_dir должна быть строка, а тут ---> {type(data_dir)}")
    ...
    def load_dataset(file_dir, file, **kwargs):
        ...
        # MIME тип файла (https://wp-kama.ru/id_8643/spisok-rasshirenij-fajlov-i-ih-mime-tipov.html)
        ...
        file_mime_type = mime.from_file(os.path.join(file_dir, file))
    ...
    def get_dataframe_from_file(self, file=None, **kwargs):
        ...
        check_file = self.check_type(file)
        # вызов статического метода проверки читаемости файла
        flag = self.is_accessible(file, self.data_dir)
        # если оба True
        if all([check_file, flag]):
            ...
            dataset = self.load_dataset(self.data_dir, file, **kwargs)
    ...
    def get_dataframe_info(self, dataset=None, info=True, miss_values=True, sample=True, num_samples=5):
        """
        ...
        - sample (bool): выводить на экран строки датасета методом sample (по умолчанию True).
        - num_samples (int): количесвто строк для вывода методом sample (по умолчанию 5).
        """
        ...
        # если выбран флаг sample, выводим на экран строки методом sample в количестве num_samples
        if sample:
            display(dataset.sample(num_samples))